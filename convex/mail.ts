...
export async function sendPasswordResetEmail(
  ...
  if (!hasResendApiKey()) {
    ...
    console.log(
      `${RESEND_LOG_PREFIX} Password reset for ${email}: ${resetUrl}`,
    );
    return;
  }
...
function buildPasswordResetEmail(args: {
  name?: string | null;
  resetUrl: string;
}) {
  ...
  const intro =
    "We received a request to reset the password for your StoreAI admin account.";
