		...
		class ProductInfo extends HTMLElement {
			...
			fetchSectionHtml() {
				...
				const sectionId = this.dataset.originalSection
					...
					: this.dataset.section;
				return fetch(
					`${this.dataset.url}?variant=${this.currentVariant.value}&section_id=${sectionId}`,
				)
			...
			syncProductInfo(sectionId, html) {
				...
				const installmentDestination = document.getElementById(
				...
				if (productFormSource && productFormDestination) {
					...
					if (sourceVariantId && destinationVariantId) {
						...
						destinationVariantId.toggleAttribute(
							"disabled",
							sourceVariantId.hasAttribute("disabled"),
						);
					}
