...
export const Route = createFileRoute("/_chrome/internal/shops")({
	validateSearch: validateInternalShopsSearch,
	component: InternalShopsRoute,
});
	...
	return (
		<ResourcePageLayout
			badges={
				<>
					...
					<StatusPill tone="neutral">{`${shopsQuery.data?.records.length ?? 0} rows`}</StatusPill>
				</>
			}
		...
		>
			...
			{shopsQuery.isPending ? (
				<Text>Loading shop records…</Text>
			) : shopsQuery.isError || !shopsQuery.data ? (
				<Text className="text-red-600 dark:text-red-500">Failed to load shop records.</Text>
			) : (
				<ResourceTable
					columns={shopColumns}
					...
					onNext={
						shopsQuery.data.pageInfo.continueCursor
							? () => {
									void navigate({
										search: (current) =>
											advanceInternalPage(
					...
					pageInfo={shopsQuery.data.pageInfo}
					rows={shopsQuery.data.records}
				/>
			)}
		</ResourcePageLayout>
	);
