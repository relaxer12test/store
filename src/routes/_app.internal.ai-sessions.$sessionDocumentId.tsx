	...
	return (
		<InternalDetailCard title={session.shopName}>
			<div className="flex flex-wrap items-center gap-2">
				<InternalStatusValue value={session.threadStatus} />
				<InternalStatusValue value={session.lastReplyTone ?? "no reply"} />
			...
			<DescriptionList>
				...
				<DescriptionDetails>
					<InternalCodeValue value={session.shopDomain} />
				</DescriptionDetails>
				<DescriptionTerm>Session id</DescriptionTerm>
				<DescriptionDetails>
					<InternalCodeValue value={session.sessionId} />
				</DescriptionDetails>
				<DescriptionTerm>Thread id</DescriptionTerm>
				<DescriptionDetails>
					<InternalCodeValue value={session.threadId} />
				</DescriptionDetails>
				<DescriptionTerm>Client fingerprint</DescriptionTerm>
				<DescriptionDetails>
					<InternalCodeValue value={session.clientFingerprint} />
				</DescriptionDetails>
				<DescriptionTerm>Created</DescriptionTerm>
				<DescriptionDetails>{formatInternalTimestamp(session.createdAt)}</DescriptionDetails>
				<DescriptionTerm>Updated</DescriptionTerm>
				<DescriptionDetails>{formatInternalTimestamp(session.updatedAt)}</DescriptionDetails>
				<DescriptionTerm>Last prompt</DescriptionTerm>
				<DescriptionDetails>{formatInternalTimestamp(session.lastPromptAt)}</DescriptionDetails>
				...
				{transcriptQuery.data.messages.length === 0 ? (
					...
					<div className="mt-3 space-y-3">
						{transcriptQuery.data.messages.map((message) => (
							...
							>
								<div className="flex flex-wrap items-center gap-2">
									<InternalStatusValue value={message.role} />
									<InternalStatusValue value={message.status} />
									<Text className="text-xs text-zinc-500 dark:text-zinc-400">
										{formatInternalTimestamp(message.createdAt)}
									</Text>
								</div>
